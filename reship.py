...
# 상품명 + 규격. 예: 방토3팩, 바질500g, 와일드1k, 고수1단, 아보카도4알
# 규격 단위 바로 뒤에 한글/영문이 이어지면 상품으로 보지 않습니다.
# 예: "수원아이파크시티2단지"의 "2단"은 상품 규격이 아니라 주소/건물명 일부이므로 제외됩니다.
_PRODUCT_RE = re.compile(
    r"(?P<name>[A-Za-z가-힣][A-Za-z가-힣·ㆍ_-]{0,30})\s*"
    r"(?P<spec>\d+(?:\.\d+)?\s*(?:kg|KG|Kg|k|K|키로|킬로|g|G|그램|팩|개|통|단|봉|박스|알))"
    r"(?![A-Za-z가-힣])"
)
