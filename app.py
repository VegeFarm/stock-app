...
def render_inventory_page():
    ...
    # -------------------- 📷 OCR (재고관리에서만 표시) --------------------
    st.divider()
    st.markdown("### 📷 캡처 이미지(OCR)로 불러오기")

    with st.expander("📷 OCR 읽기 (재고관리 전용)", expanded=False):
        if pytesseract is None:
            st.error("OCR을 사용하려면 pytesseract 설치가 필요합니다. (pip install pytesseract)")
        elif Image is None:
            st.error("OCR을 사용하려면 Pillow 설치가 필요합니다. (pip install pillow)")
        else:
            # rules.txt 로드(제품별 합계 변환 규칙 동일 적용)
            ...
            with colU1:
                ocr_img = st.file_uploader("캡처 이미지 업로드(PNG/JPG)", type=["png", "jpg", "jpeg"], key="inv_ocr_uploader")
            with colU2:
                st.text_input(
                    "tesseract 경로(윈도우, 선택)",
            ...
            with st.expander("⚙️ OCR 옵션(정확도 조정)", expanded=False):
                lang = st.selectbox("언어", ["kor+eng", "kor", "eng"], index=0, key="inv_ocr_lang")
                psm = st.selectbox("PSM(레이아웃)", [6, 4, 11, 3], index=0, key="inv_ocr_psm")
                scale = st.slider("확대(선명도)", 1.0, 4.0, 2.5, 0.1, key="inv_ocr_scale")
                contrast = st.slider("대비", 1.0, 3.5, 2.0, 0.1, key="inv_ocr_contrast")
                sharp = st.slider("선명도", 1.0, 3.5, 1.5, 0.1, key="inv_ocr_sharp")
                thresh = st.slider("이진화(0=끄기)", 0, 255, 0, 1, key="inv_ocr_thresh")
                invert = st.checkbox("흑백 반전", value=False, key="inv_ocr_invert")

                allow_decimal_pack = st.checkbox("팩 소수 허용", value=False, key="inv_allow_decimal_pack")
                allow_decimal_box = st.checkbox("박스 소수 허용", value=True, key="inv_allow_decimal_box")
            ...
            with btn1:
                do_ocr = st.button("🔎 OCR 읽기", use_container_width=True, key="inv_do_ocr_btn", disabled=(ocr_img is None))
            with btn2:
                do_clear = st.button("🧹 OCR 초기화", use_container_width=True, key="inv_ocr_clear_btn")
            ...
            if do_ocr and ocr_img is not None:
                ...
                try:
                    ...
                    st.success("OCR 읽기 완료! 아래 텍스트를 필요하면 수정한 뒤 합산을 누르세요.")
                except Exception as e:
                    st.error(f"OCR 실패: {e}")

            # OCR 결과(수정 가능)
            st.text_area("OCR 결과(수정 가능)", key="inv_ocr_text", height=220)

            csum1, csum2 = st.columns([1, 1])
            with csum1:
                do_sum = st.button("🧾 이 텍스트로 합산", use_container_width=True, key="inv_ocr_sum_btn")
            with csum2:
                reg_target = st.selectbox("재고표 등록 대상", ["입고", "1차", "2차", "3차"], index=1, key="inv_ocr_reg_target")

            add_mode = st.checkbox("기존 값에 누적(더하기)", value=False, key="inv_ocr_add_mode")
            ...
            if isinstance(df_long_ocr, pd.DataFrame) and len(df_long_ocr) > 0:
                ...
                with regC1:
                    do_reg = st.button("✅ 재고표에 등록", use_container_width=True, key="inv_ocr_register_btn")
                with regC2:
                    st.caption("※ 재고관리 표에 **이미 존재하는 상품명만** 반영됩니다. (없는 상품은 제외)")

                if do_reg:
                    updated, skipped = register_sum_to_inventory(df_long_ocr, target_col=reg_target, add_mode=add_mode)

                    # 계산값(Disabled 컬럼) 즉시 반영되도록 에디터 키 갱신
                    ...
                    st.success(f"{reg_target}에 등록 완료! (반영 행: {updated})")
                    st.rerun()
