...
def render_bulk_stock_page():
    ...
    # Auto mode helpers
    # ============================
    NAVER_COMMERCE_BASE_URL = (os.environ.get("NAVER_COMMERCE_BASE_URL") or "https://api.commerce.naver.com/external").strip().rstrip("/")
    NAVER_COMMERCE_CLIENT_ID = (os.environ.get("NAVER_COMMERCE_CLIENT_ID") or "").strip()
    NAVER_COMMERCE_CLIENT_SECRET = (os.environ.get("NAVER_COMMERCE_CLIENT_SECRET") or "").strip()
    TELEGRAM_BOT_TOKEN = (os.environ.get("TELEGRAM_BOT_TOKEN") or "").strip()
    TELEGRAM_CHAT_ID = (os.environ.get("TELEGRAM_CHAT_ID") or "").strip()
    TELEGRAM_AUTO_POLL_SECONDS = max(2, int((os.environ.get("TELEGRAM_AUTO_POLL_SECONDS") or "5").strip() or "5"))
    NAVER_PRODUCT_SEARCH_PAGE_SIZE = max(1, min(500, int((os.environ.get("NAVER_PRODUCT_SEARCH_PAGE_SIZE") or "500").strip() or "500")))
    NAVER_PRODUCT_SEARCH_MAX_PAGES = max(1, min(50, int((os.environ.get("NAVER_PRODUCT_SEARCH_MAX_PAGES") or "10").strip() or "10")))
    ...
    def _naver_token(force_refresh: bool = False) -> str:
        cache = st.session_state.get("bulk_naver_token_cache") or {}
        now_ts = time.time()
        if (not force_refresh) and cache.get("access_token") and float(cache.get("expires_at", 0)) > now_ts + 60:
        ...
        if resp.status_code >= 400:
            raise RuntimeError(f"네이버 토큰 발급 실패: {resp.status_code} / {_safe_json(resp)}")
        data = resp.json()
    ...
    def _naver_request(method: str, path: str, *, json_body: Any = None, params: Optional[Dict[str, Any]] = None, retry: bool = True):
        token = _naver_token(False)
        headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
        url = f"{NAVER_COMMERCE_BASE_URL}{path}"
        resp = requests.request(method.upper(), url, headers=headers, json=json_body, params=params, timeout=60)
        if retry and resp.status_code == 401:
            token = _naver_token(True)
            headers["Authorization"] = f"Bearer {token}"
            resp = requests.request(method.upper(), url, headers=headers, json=json_body, params=params, timeout=60)
        if resp.status_code >= 400:
            raise RuntimeError(f"네이버 API 실패: {method.upper()} {path} / {resp.status_code} / {_safe_json(resp)}")
    ...
    def fetch_naver_products_df() -> pd.DataFrame:
        ...
        for page in range(1, NAVER_PRODUCT_SEARCH_MAX_PAGES + 1):
            body = _get_search_body(page, NAVER_PRODUCT_SEARCH_PAGE_SIZE)
            data = _naver_request("POST", "/v1/products/search", json_body=body)
            df_page = _extract_products_from_search_payload(data)
            if df_page.empty:
                break
            frames.append(df_page)
            if len(df_page) < NAVER_PRODUCT_SEARCH_PAGE_SIZE:
                break
        if not frames:
            raise RuntimeError("네이버 상품 조회 결과가 없습니다. NAVER_PRODUCT_SEARCH_BODY 환경변수를 확인해 주세요.")
    ...
    def build_multi_update_payload(df_changes: pd.DataFrame) -> Dict[str, Any]:
        ...
        if df_changes is None or df_changes.empty:
            return {"multiProductUpdateRequestVos": []}
        for _, row in df_changes.iterrows():
            try:
                ...
                continue
            items.append({"originProductNo": origin_no, "multiUpdateTypes": ["STOCK_QUANTITY"], "stockQuantity": qty})
        return {"multiProductUpdateRequestVos": items}

    def push_stock_updates(df_changes: pd.DataFrame) -> Dict[str, Any]:
        payload = build_multi_update_payload(df_changes)
        if not payload.get("multiProductUpdateRequestVos"):
            return {"sent": 0, "response": {"message": "변경 대상 없음"}}
        data = _naver_request("PATCH", "/v1/products/origin-products/multi-update", json_body=payload)
        return {"sent": len(payload.get("multiProductUpdateRequestVos") or []), "response": data, "payload": payload}
